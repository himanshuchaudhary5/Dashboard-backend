...
public interface OverAllExecutionRepository extends JpaRepository<OverAllExecution,Integer>, JpaSpecificationExecutor<OverAllExecution> {
    ...
    @Query(nativeQuery = true,value = "Select ROW_NUMBER() over() as id, passed, failed, skipped from (\n" +
            "(Select count(*) as passed from test_case where failed = 0 and skipped = 0 and passed > 0)  as passcount,\n" +
            "(Select count(*) as failed from test_case where failed > 0 )  as failcount,\n" +
            "(Select count(*) as skipped from test_case where failed = 0 and passed = 0 and skipped > 0 )  as skipcount\n" +
            ");")
